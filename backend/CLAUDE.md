...
| `Mutation.requestBodyAssessment(notes)` / `Query.myAssessmentRequests` / `Query.assessmentRequests(status)` | required | Student asks the academy for a body assessment (content type `BodyAssessmentRequest`, status `pending/done/cancelled`). Creating one notifies the admins (`admin_assessment_request`) and is idempotent (reuses an open request). Auto-resolved to `done` when `createBodyAssessment` runs for that student. `assessmentRequests` is staff-only (`academy_admin`/`instructor`); the admin sees a pending banner in `/admin/workouts → Avaliações`. See design-decisions §2.20. |
